...
export class WorkspaceService extends EventEmitter implements WorkspaceHost {
  ...
  private bumpContextMutationEpoch(workspaceId: string): void {
  ...
  private async advanceContextMutationEpoch(
    ...
    this.bumpContextMutationEpoch(workspaceId);
    try {
      ...
      // reporting a cancellation-retirement failure or releasing admission.
      return Err(
        `History changed, but canceled compaction state could not be retired: ${getErrorMessage(error)}`
      );
    }
  }
  ...
  private createSession(workspaceId: string): AgentSession {
    ...
    return new AgentSession({
      ...
      onContextWindowRollover: () => this.bumpContextMutationEpoch(workspaceId),
  ...
  async replaceHistory(
    ...
    try {
      ...
      let cancellationRetirement: Result<void> = Ok(undefined);
