...
export class WorkspaceService extends EventEmitter implements WorkspaceHost {
  ...
  private async materializeDeferredCheckout(args: {
    ...
    // Only removal may interrupt the origin sync and file checkout themselves: archive aborts
    // init too but keeps the checkout registered and never reruns it, so parking a
    // half-populated worktree would strand it. Archive awaits this settlement, so it parks
    // complete files; every phase after them (hooks, .xumignore, submodules) honours its abort.
