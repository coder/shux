...
describe("AgentSession token-budget lifecycle", () => {
  ...
  test.each([false, true])(
    "on-send rollover preserves the atomic request and its own handoff (%s)",
    async (handoff) => {
      const h = await setup();
      await seedHistory(h, 110_000);
      const coordinator = (h.session as unknown as { coordinator: TurnCoordinator }).coordinator;
      const oldObservation = coordinator.beginCompactionObservation("continuous");
      ...
      const result = await h.session.sendMessage(
        ...
        handoff
          ? {
              synthetic: true,
              compactionHandoff: oldObservation!,
              admissionStale: () => !coordinator.isCurrentCompaction(oldObservation!),
            }
          : undefined
      );
      expect(result.success).toBe(true);
      expect(coordinator.isCurrentCompaction(oldObservation!)).toBe(handoff);
      const rows = await allRows(h);
  ...
  test("explicit Retry consumes its Stop receipt once across emergency budget rollover", async () => {
    const h = await setup({ failure: (attempt) => (attempt === 1 ? exceeded : undefined) });
    await seedHistory(h, 20_000);
    await h.historyService.appendToHistory(
      workspaceId,
      createMuxMessage("retry-user", "user", "Resume work")
    );
    await h.session.interruptStream({ abandonPartial: true });
    const accept = spyOn(h.historyService, "acceptResumeCancellation");
